...
class SMACClient():
    ...
    async def recv_loop(self, *args):
        await asyncio.sleep(0)
        while True:
            readers, writers, exceptions = select.select([self.udp_sock, self.zmq_sub_reader], [], [])
            for reader in readers:
                if reader is self.udp_sock:
                    try:
                        data, addr = self.udp_sock.recvfrom(self.MAX_BUFFER)
                        d = data.decode("utf-8")
                        self.UDP_REQ.append(d)
                    except Exception as e:
                        print("sel_err --> udp read", e)
                    await asyncio.sleep(0)
                elif reader is self.zmq_sub_reader:
                    try:
                        ...
                        print("sel_err --> zmq read", e)
                    await asyncio.sleep(0)


    async def on_msg_and_send_msg_loop(self, *args):
        await asyncio.sleep(0)
        while True:
            await self.parse_zmq_msg()
            await self.parse_udp_msg()
            await self.send_message_listener_zmq()
    ...
    async def main(self, *args):
        ...
        t1 = asyncio.create_task( self.recv_loop() )
        t2 = asyncio.create_task( self.on_msg_and_send_msg_loop() )
